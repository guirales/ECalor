...
File logFile;
int pinSD, reintento;
String EntradaN, nombreArch, Taux;
char SalidaN[50];
...
void setup() {
  ...
  TempE = 20;
  Pot = 0;
  est = 0;//0 para potencia 1 para temperatura
  bandera = true;
...
void loop() {
  ...
  while (Dtf <= Dt * 1000) {
    ...
    Serial.println(equipo + "," + nombreA + "," + String(TempE) + "," + String(n * Dt) + "," + String(estado) + "," + String(TempA) + "," + String(ImA) + "," + String(st1) + "," + String(st2) + "," + String(st3) + "," + String(st4));
...
void LectoEscrituraNextion(void)
{
  ...
  //
  delay(80);
  EntradaN = "Medida.Entrada.txt=\"" + String(Dt * n) + "," + String(n) +","+ String(TempA)+","+String(ImA)+","+String(st1)+","+String(st2)+","+String(st3)+","+String(st4)+"; \"";
  ...
  if(tamN != 0) {    
    ...
    for (i = 0; i <= tamN; i++) {
      ...
      if (SalidaN[i] == ',') {
        ...
        Taux += SalidaN[i];
        if(n == tamN) {
          Serial.println("taux: " + String(Taux));
        }
      }
    }
    Serial.println("POTENCIAL: " + String(Pot));
    Pot = Taux.toInt();
  }else {
    Serial.println("POTENCIAL: " + String(Pot));
  }
